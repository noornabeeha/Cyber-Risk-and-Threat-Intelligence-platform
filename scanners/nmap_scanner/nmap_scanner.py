import subprocess # Run system/shell commands from Python
import xml.etree.ElementTree as ET #(ET) – Parse and manipulate XML files
from xml.etree.ElementTree import Element, SubElement #Create XML elements programmatically
from xml.dom import minidom # Pretty-print XML with proper indentation
import os # Interact with the file system (paths, env vars, dirs)
...
def run_nmap_scan(target: str) -> str:
    ...
    cmd = [
        ...
        "-sT",
        "-O",
        "-sV", "--version-intensity", "5", #So higher intensity = more probes sent = slower scan but better version info.
        "--script", "banner",          # grab banners where possible -- When you connect to most network services, 
        #they immediately send you a greeting message before you even do anything. This is called a banner.
        "-oX", xml_file,
        target,
    ]
    ...
    for host in root.findall("host"):
        ...
        for port in host.findall(".//port"):
            ...
            if tunnel == "ssl" and risk == "high":
                risk        = "medium"
                risk_reason = f"{risk_reason} (mitigated by SSL)" #Secure Sockets Layer
